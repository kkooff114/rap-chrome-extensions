function JsonFormater(opt) {
    var Collapsed = chrome.extension.getURL("images/Collapsed.gif")
    var Expanded = chrome.extension.getURL("images/Expanded.gif")
    this.options = $.extend({
        dom: '',
        tabSize: 2,
        singleTab: "  ",
...
JsonFormater.prototype = {
    ...
    makeContentVisible: function (element, visible) {
        ...
        if (visible) {
            ...
            img.src = this.options.imgExpanded;
            img.data('status', 1);
        } else {
            element.hide();
            //img.attr('src', this.options.imgCollapsed);
            img.src = this.options.imgCollapsed;
            img.data('status', 0);
        }
    },
